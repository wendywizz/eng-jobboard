...
class jobsearch_meeting_cancel_email_to_employer_template {
    ...
    public function __construct() {
        ...
        add_action('init', array($this, 'jobsearch_meeting_cancel_email_to_employer_template_init'), 1, 0);
        add_filter('jobsearch_meeting_cancel_email_to_employer_filter', array($this, 'jobsearch_meeting_cancel_email_to_employer_filter_callback'), 1, 4);
        add_filter('jobsearch_email_template_settings', array($this, 'template_settings_callback'), 12, 1);
        add_action('jobsearch_meeting_cancel_email_to_employer', array($this, 'jobsearch_meeting_cancel_email_to_employer_callback'), 10, 3);
    }

    public function jobsearch_meeting_cancel_email_to_employer_template_init() {
        $this->user = array();
        $this->rand = rand(0, 99999);
        $this->group = 'job';
        $this->type = 'meeting_cancel_email_to_employer';
        $this->filter = 'meeting_cancel_email_to_employer';
        $this->email_template_db_id = 'meeting_cancel_email_to_employer';
        $this->switch_label = esc_html__('Canceled Meeting to Employer', 'jobsearch-shmeets');
        $this->default_subject = esc_html__('Canceled Meeting by Candidate', 'jobsearch-shmeets');
        $this->default_recipients = '';
        $default_content = esc_html__('Default content', 'jobsearch-shmeets');
        $default_content = apply_filters('jobsearch_meeting_cancel_email_to_employer_filter', $default_content, 'html', 'job-cancel-meeting', 'email-templates');
        $this->default_content = $default_content;
        $this->email_template_prefix = 'meeting_cancel_email_to_employer';
        $this->email_template_group = 'employer';
        $this->codes = apply_filters('jobsearch_meetin_cancel_email_toemp_codes', array(
    ...
    public function jobsearch_meeting_cancel_email_to_employer_callback($user = '', $meeting_id = '', $job_id = '') {
        ...
        if (isset($template['switch']) && $template['switch'] == 1) {
            ...
            $subject = (isset($template['subject']) && $template['subject'] != '' ) ? $template['subject'] : __('Canceled Job Meeting', 'jobsearch-shmeets');
            ...
            jobsearch_meeting_cancel_email_to_employer_template::$is_email_sent1 = $this->is_email_sent;
        }
    }
    ...
    public function jobsearch_meeting_cancel_email_to_employer_filter_callback($html, $slug = '', $name = '', $ext_template = '') {
        ob_start();
        $html = '';
        $template = '';
        if ($ext_template != '') {
    ...
    public function template_settings_callback($email_template_options) {
        ...
        $email_template_options['meeting_cancel_email_to_employer']['rand'] = $this->rand;
        $email_template_options['meeting_cancel_email_to_employer']['email_template_prefix'] = $this->email_template_prefix;
        $email_template_options['meeting_cancel_email_to_employer']['email_template_group'] = $this->email_template_group;
        $email_template_options['meeting_cancel_email_to_employer']['default_var'] = $this->default_var;
...
new jobsearch_meeting_cancel_email_to_employer_template();
